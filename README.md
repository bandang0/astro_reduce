...
>The FITS files for object _obj_, with filter _filt_ and exposure time _exp_ are found in the `ar_tmp/` folder after reduction, under the name `[obj]_[filt]_[exp]_*_aux.fits` (for _auxiliary_), which contain `astro_reduce`-internal reference numbers.
...
>These stacked images are the final cosmetically reduced images and can be found in the `stacked/` folder after reduction, with the same names as in the preceding step, save the `_aux` suffix and the internal image reference numbers.
...
- Using `--psfex` (which implies `--sex`) runs the `PSFEx` program on the auxiliary images with `sex`'s results to produce PSF-related results in the `PSFRES/` folder.
...
> If the `--scamp` option was used, the final astrometrically reduced files with updated headers can be found in `reduced/` after run. By a magical trick, these final files have the same names as the original object images in the `ORIGINAL/` folder.
